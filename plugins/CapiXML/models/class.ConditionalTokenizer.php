...
Fais afficher le message :

Notice: Undefined offset: -1 in /home/chris/documents/crp/src/tao/plugins/CapiXML/models/class.ConditionalTokenizer.php on line 351

Catchable fatal error: Argument 1 passed to ListOfToken::del() must be an instance of Token, null given, called in /home/chris/documents/crp/src/tao/plugins/CapiXML/models/class.ConditionalTokenizer.php on line 360 and defined in /home/chris/documents/crp/src/tao/plugins/CapiXML/models/class.ConditionalTokenizer.php on line 84


==================================
if (or B_Q02bDE2=DK or B_Q02bDE2=RF or B_Q02b3DE1=RF or B_Q02b3DE2=RF) B_D02b3DE1=RF,
==================================
...
class ListOfToken implements IteratorAggregate {
  ...
  function sortStickness() {
    usort($this->list, array("Token", "cmpSticknessRevert"));
/*
...
class Token { // implements IteratorAggregate {
...
/*    echo "Mi estas '".get_class($this)."' kiu enhavas '$this'";
    echo ", mi havas ".$this->children->count()." filo(j)n\n";
*/
  ...
  function getXmlOperator($type) {
    ...
    foreach ($this->children as $child) {
      $operator->appendChild(
        $dom->importNode($child->getXml()->documentElement, true)
      );
    }
    return $dom;
  }
...
/*      $left = "{$this->children->get(0)}";
  ...
  /** Reduction function. By defaut, the token is ignored/deleted */
  ...
  static function cmpSticknessRevert($left, $right) {
    ...
    if ($left->stickness < $right->stickness)
      return +1;
    else if ($left->stickness > $right->stickness)
      return -1;
    else
      return 0;
  }
    ...
    if (!empty($this->allowedChildren)) {
      ...
      foreach($this->allowedChildren as $childTypes) {
        ...
        if (!in_array($childType, $childTypes)) {
          throw new Exception(
            "Type error: $childType in ".get_class($this)."."
          );
        }
        if (++$index>=$count) break; // if there are optional children
      }
    }
  }
...
//     print_r($tokens->list);
    ...
    if (FALSE === $pos) throw new Exception("Token to reduce not found");
...
//     print_r($right);
    ...
    $this->children = new ListOfToken(array($left, $right));
    $tokens->del($left);
    $tokens->del($right);
  }
...
//     print_r($right);
    ...
    $this->children = new ListOfToken(array($right));
    $tokens->del($right);
  }
...
//         print_r($listOfToken);
  ...
  function correctMissingThenElse() {
    ...
    if (   !is_null($childElseElseIf)
        && !$childElseElseIf->typeOf($tokenElse)
        && !$childElseElseIf->typeOf($tokenElseIf)
    ) {
  ...
  function reduce(&$tokens) {
    ...
    $this->children = new ListOfToken(array($condition));
    ...
    $listOfToken = new ListOfToken(array($conditionOrig));
...
//      echo "@ '$token'\n";
      ...
      if ($token->typeOf($this)) {
        $level++;
        //echo "i+\n";
      } else if ($token->typeOf($tokenCloseParenthesis)) {
        $level--;
        //echo "i-\n";
      }
      if (0==$level) {
...
class TokenExpr extends Token {
  ...
  function getXml($name = null) {
    ...
    if ($xpath->query("//goto")->length>0) {
      $keyWord = "routing";
    } else if ($xpath->query("//assignment")->length>0) {
      $keyWord = "inferenceRule";
    } else if (!$xpath->query("//then")->length>0) {
...
//     exit();
  ...
  function __toString() { return self::$regexp; }
